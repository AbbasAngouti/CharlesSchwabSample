/*
    ...
    Abstract:
    A class used to populate a `ProductItemCollectionViewCell` for a given `DataItem`. The composer class handles processing and caching images for `DataItem`s.
*/
...
class DataItemCellComposer {
    ...
    */
    private var operationQueues = [ProductItemCollectionViewCell: NSOperationQueue]()

    // MARK: Implementation
    
    func composeCell(cell: UICollectionViewCell, withDataItem dataItem: AnyObject, type: String) {
        // Cancel any queued operations to process images for the cell.
        if let cell = cell as? ProductItemCollectionViewCell, dataItem = dataItem as? ProductOnlyItem {
            ...
            if let titleLabel = cell.titleLabel {
                titleLabel.text =  dataItem.title//.decodeEnt()
            }
            if let instructorLabel = cell.instructorLabel {
                instructorLabel.text = "Instructor: \(dataItem.instructor)"
            }
            if let genreLabel = cell.genreLabel {
                genreLabel.text = "Genre: \(dataItem.genre)"
            }
            if let guitarTypeLabel = cell.guitarTypeLabel {
                guitarTypeLabel.text = "Guitar Type: \(dataItem.guitarType)"
            }
            cell.imageView.alpha = 1.0
            cell.imageView.image = DataItemCellComposer.processedImageCache.objectForKey(dataItem.uuid+type) as? UIImage
            ...
            */
            ...
            processImageOperation.addExecutionBlock { [unowned processImageOperation] in
                ...
                ImageLoader.sharedLoader.imageForUrl(dataItem.thumbnailUrl, completionHandler: { (image, url) -> () in
                    ...
                    DataItemCellComposer.processedImageCache.setObject(image!, forKey: dataItem.uuid+type)
    ...
    */
    private func operationQueueForCell(cell: ProductItemCollectionViewCell) -> NSOperationQueue {
